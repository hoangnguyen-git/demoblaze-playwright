...
test.describe('Login', {
    ...
    test('Verify SQL Injection: \' OR \'1\'=\'1', async ({ loginPage }) => {
        await loginPage.goto();
        await loginPage.clickLoginOnMenuBar();
        loginPage.page.once('dialog', (dialog) => {
            expect(dialog.message()).toContain(LoginConstants.wrongUsernameMessage);
        ...
        await loginPage.login("' OR '1'='1", Constants.PASSWORD);
    });

    test('Verify XSS injection in input fields', async ({ loginPage }) => {
        await loginPage.goto();
        await loginPage.clickLoginOnMenuBar();
        loginPage.page.once('dialog', (dialog) => {
            expect(dialog.message()).toContain(LoginConstants.wrongUsernameMessage);
        ...
        await loginPage.login('<script>alert(1)</script>', Constants.PASSWORD);
    });
},
);
